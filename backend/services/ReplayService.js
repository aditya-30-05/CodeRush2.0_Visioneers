...
export async function listReplayMissions() {
  ...
  // 1. Process DB missions
  for (const m of dbMissions) {
    const id = m.mission_id;
    if (!id || processedIds.has(id)) continue;
    processedIds.add(id);

    const [missionRec, telRows, faultRows] = await Promise.all([
      MissionRepository.findById(id),
      TelemetryRepository.findByMission(id, 1000, 0),
      FaultRepository.findByMission(id),
    ]);

    const duration = telRows.length > 0
      ? ((telRows[telRows.length - 1].mission_time || 0) - (telRows[0].mission_time || 0))
      : 0;

    results.push({
      missionId:      id,
      missionName:    missionRec?.mission_name || `OrbitOps Mission ${id.slice(0, 8)}`,
      startTime:      missionRec?.start_time || telRows[0]?.timestamp || new Date().toISOString(),
      endTime:        missionRec?.end_time || telRows[telRows.length - 1]?.timestamp || new Date().toISOString(),
      duration:       duration,
      snapshotsCount: telRows.length,
      eventsCount:    (faultRows.length || 0) + 2,
      faultsCount:    faultRows.length || 0,
      status:         missionRec?.status || 'COMPLETED',
    });
  }

  // 2. Include active in-memory session if not already in DB
  if (currentStatus?.missionId && !processedIds.has(currentStatus.missionId)) {
    const memBuffer = SimulationService.getTelemetryBuffer();
    if (memBuffer.length > 0) {
      results.push({
        missionId:      currentStatus.missionId,
        missionName:    currentStatus.missionName || 'Active Session Mission',
        startTime:      currentStatus.startedAt || new Date().toISOString(),
        endTime:        new Date().toISOString(),
        duration:       currentStatus.missionTime || 0,
        snapshotsCount: memBuffer.length,
        eventsCount:    (_liveSessionEvents.get(currentStatus.missionId)?.length || 0),
        faultsCount:    memBuffer.filter(t => t.faults?.length > 0).length,
